...
def get_provider_logo_base64(provider_name: str) -> str:
    ...
    logos_dir = Path(__file__).parent.parent / "data" / "provider_logos"
...
def get_recommendations_db(titles, num_of_rec=16, genre_filter=None, media_type='movies'):
    ...
    for rec in recs:
        ...
        # providers = []
        # if rec.get('watch_providers'):
        #     providers = parse_watch_providers_from_json(rec['watch_providers'], region)
        # else:
        #     # Fallback: query database if not in result
        #     providers = get_watch_providers_from_db(rec['id'], media_type, region)
